...
contract Referrals is IReferrals, Ownable {
  ...
  event RegisteredName(address owner, bytes32 name);
  
  /// @notice Referrer names for user friendly referrer picking
  mapping (bytes32 => address) private _nameToAddresses;
  mapping (address => bytes32) private _addressToNames;
  ...
  function registerName(bytes32 name) public {
    require(name != bytes32(0), "Empty name");
    require(_nameToAddresses[name] == address(0x0) && _addressToNames[msg.sender] == bytes32(0), "Already registered");
    _nameToAddresses[name] = msg.sender;
    _addressToNames[msg.sender] = name;
    emit RegisteredName(msg.sender, name);
  }
  
  /// @notice Register a referrer by name
  function registerReferrer(bytes32 name) public {
    address referrer = _nameToAddresses[name];
  ...
  function getReferralParameters(address user) external view 
    returns (bytes32 name, uint referreesLength, address _referrer, uint16 _rebateReferrer, uint16 _discountReferee) 
  {
    name = _addressToNames[user];
    referreesLength = _referrerToReferees[user].length;
