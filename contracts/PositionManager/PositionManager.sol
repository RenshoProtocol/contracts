...
contract PositionManager is Commons, ERC721Enumerable, IPositionManager {
  ...
  constructor() ERC721("Rensho Positions", "RP") {}
  ...
  function tokenURI(uint256 tokenId) public view override(ERC721, IERC721Metadata) returns (string memory) {
    ...
    (, uint pnl) = getValueAtStrike(position.isCall, IVault(vault).getBasePrice(), position.strike, position.notionalAmount);
  ...
  function openFixedPosition(bool isCall, uint strike, uint notionalAmount, uint timeToExpiry) external returns (uint tokenId){
    ...
    uint basePrice = IVault(vault).getBasePrice();
  ...
  function openPosition(bool isCall, uint strike, uint notionalAmount, uint collateralAmount, uint timeToExpiry) internal returns (uint tokenId) {
    ...
    if (collateralAmount > 0) optionCost = 2 * optionCost;
    ...
    _positions[tokenId] = Position(
      isCall,
      isStreamingOption ? IPositionManager.OptionType.StreamingOption : IPositionManager.OptionType.FixedOption, 
      strike, 
      notionalAmount, 
      collateralAmount,
      block.timestamp, 
    ...
    IVault(vault).borrow(address(isCall ? baseToken : quoteToken), notionalAmount);
  ...
  require(_positions[tokenId].optionType == IPositionManager.OptionType.StreamingOption, "GEP: Not Streaming Option");
  ...
  function closePosition(uint tokenId) external {
    ...
    require(
      msg.sender == owner
        || (position.optionType == IPositionManager.OptionType.StreamingOption && feesDue >= position.collateralAmount - FIXED_EXERCISE_FEE)
        || (position.optionType == IPositionManager.OptionType.FixedOption && block.timestamp >= position.data )
    ...
    (uint vaultDue, uint posPnl) = getValueAtStrike(position.isCall, IVault(vault).getBasePrice(), position.strike, position.notionalAmount);
    ...
    IVault(vault).repay(positionToken, vaultDue, feesDue - feesRebate);
    ...
    if (owner != msg.sender) {
      quoteToken.safeTransfer(IController(IVaultConfigurator(vault).controller()).treasury(), FIXED_EXERCISE_FEE / 2);
  ...
  function getFeesAccumulatedAndMin(uint tokenId) public view returns (uint feesAccumulated,  uint feesMin) {
    ...
    if (position.optionType == IPositionManager.OptionType.StreamingOption){
  ...
  function getAssetsDue() public view returns (uint baseAmount, uint quoteAmount) {
    if (openStrikes.length > 1){
      uint price = IVault(vault).getBasePrice();
  ...
  function getOptionCost(bool isCall, uint strike, uint notionalAmount, uint timeToExpirySec) public view returns (uint optionCost){
    ...
    (uint baseBalance, uint quoteBalance) = IVault(vault).getAdjustedReserves();
  ...
  function getUtilizationRateStatus() public view returns (uint utilizationRate, uint maxOI) {
    (uint baseBalance, uint quoteBalance, ) = IVault(vault).getReserves();
  ...
  function name() public view virtual override(ERC721, IERC721Metadata) returns (string memory) { 
    return string(abi.encodePacked("Rensho Positions ", baseToken.symbol(), "-", quoteToken.symbol()));
  }
  ...
  function getReferrals() internal view returns (address) {
    return IController(IVaultConfigurator(vault).controller()).referrals();
  }
}